...
export class GenericEditPage {
  ...
  imageRequired(){
    return this.model.fields[this.imageKey].required;
  }
  getImageKey(){
    for (let key in this.model.fields){
      if (this.model.fields[key].type == 'imageb64'){
        return key;
      }
    }
    return null;
  ...
  extractIds(model, object, notIn = null){
    let result = {};
    for (let attr in model.fields){
      if (model.fields[attr].type == 'id'){
        if (object[attr]){
          if (notIn){
            if (!notIn[attr]){
  ...
  convert(data, toRaw){
    let result = {};
    for(let key in data){
      if (data[key] != null && this.model.fields[key].type == 'date'){
        if (toRaw){
      ...
      else if (data[key] != null && this.model.fields[key].type == 'multiple'){
        if (toRaw){
          let len = this.model.fields[key].values.length;
  ...
  onClickSave(){
    ...
    for(let key in this.model.fields){
      ...
      if (this.model.fields[key].required){
        if (!this.data[key]){
